...
@Component({
  ...
  template: `
    <div class="flex flex-col w-full" role="tablist">
      <!-- Tab Navigation -->
      <div
        class="w-full border-none shadow-none relative flex items-center"
        [class.bg-transparent]="themeService.theme() === 'dark'"
      >
        <div class="p-0">
          <div class="relative">
            <!-- Hover Highlight -->
            ...
            <div class="relative flex space-x-[6px] items-center">
              ...
              >
                <div
                  class="text-sm leading-5 whitespace-nowrap flex items-center justify-center h-full"
                >
                  {{ tab.label }}
                </div>
              </button>
...
export class TabsComponent implements AfterViewInit, AfterContentInit, OnInit {
  ...
  tabPanels!: ContentQueryList<TabPanelComponent>;
  ...
  ngAfterViewInit() {
    ...
    setTimeout(() => {
      const index = this.tabs.findIndex(
        (tab) => tab.id === this.activeTabIdSignal()
      );
      const elements = this.tabRefs.toArray();
      if (elements && elements[index]) {
        const element = elements[index].nativeElement;
